  ...
  return (
    <div className="min-h-screen p-6 md:p-10 font-sans text-left pb-24" style={{ backgroundImage: "url('/fondo-pacientes.png')", backgroundSize: 'cover', backgroundPosition: 'center', backgroundAttachment: 'fixed' }}>
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {/* HEADER PRINCIPAL */}
        <div className="bg-white/90 backdrop-blur-xl p-6 md:p-8 rounded-[2.5rem] shadow-xl border border-white/60 flex flex-col md:flex-row justify-between items-start md:items-center gap-6">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-2xl font-black uppercase italic tracking-tighter text-slate-800 leading-none">RX y Documentos</h3>
              <p className="text-slate-400 text-[10px] font-bold uppercase tracking-widest mt-1">Expediente digital del paciente</p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-3">
            <button 
              ...
              className={`px-6 py-3.5 rounded-2xl font-black text-[10px] uppercase transition-all flex items-center gap-2 shadow-sm ${modoSeleccion ? 'bg-blue-100 text-blue-700 border border-blue-200' : 'bg-white/90 backdrop-blur-xl border border-white/80 text-slate-700 hover:bg-white'}`}
            >
            ...
            <label className="bg-gradient-to-r from-blue-600 to-blue-700 text-white px-7 py-3.5 rounded-2xl font-black text-[10px] uppercase cursor-pointer shadow-lg shadow-blue-500/25 hover:shadow-blue-500/40 hover:from-slate-900 hover:to-slate-900 transition-all flex items-center gap-2 border border-blue-500">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-6">
          {documentos.length === 0 ? (
            <div className="col-span-full py-24 text-center bg-white/50 backdrop-blur-md rounded-[3rem] border-2 border-dashed border-slate-200 shadow-sm">
            ...
            documentos.map((doc) => (
              <motion.div 
                ...
                className={`group cursor-pointer bg-white/90 backdrop-blur-xl p-5 rounded-[2.5rem] border shadow-xl transition-all text-left relative ${modoSeleccion && seleccionMultiples.includes(doc.id) ? 'border-blue-500 ring-4 ring-blue-50' : 'border-white/80 hover:border-blue-300'}`}
              >
                ...
                <div className={`aspect-square bg-slate-100/80 rounded-[2rem] mb-4 overflow-hidden flex items-center justify-center relative transition-opacity ${modoSeleccion && seleccionMultiples.includes(doc.id) ? 'opacity-70' : ''}`}>
                ...
                <div className="px-1 text-center">
                  <p className="text-[10px] font-black text-slate-800 uppercase truncate leading-tight">{doc.titulo || doc.nombre_archivo}</p>
        ...
        <AnimatePresence>
          {modoSeleccion && seleccionMultiples.length > 0 && (
              <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: 50, opacity: 0 }} className="fixed bottom-10 left-1/2 -translate-x-1/2 bg-slate-900/95 backdrop-blur-2xl p-3 rounded-[2rem] shadow-2xl z-[99999] flex items-center gap-4 border border-white/10">
                  ...
                  <div className="flex items-center gap-2">
                      <button onClick={descargarMultiples} className="px-5 py-3 bg-emerald-600 text-white hover:bg-emerald-500 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 shadow-md">
                          <Download size={15} strokeWidth={2.5}/> Descargar
                      </button>
                      <button onClick={() => {
                      ...
                      }} className="px-5 py-3 bg-blue-600 text-white hover:bg-blue-500 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2 shadow-md">
                          <Edit3 size={15} strokeWidth={2.5}/> Renombrar
                      </button>
                      <button onClick={eliminarMultiples} className="px-5 py-3 bg-red-500/20 text-red-400 hover:bg-red-500 hover:text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center gap-2">
                          <Trash2 size={15} strokeWidth={2.5}/> Eliminar
                      </button>
                  </div>
                  <button onClick={() => { setSeleccionMultiples([]); setModoSeleccion(false); }} className="p-3 text-slate-400 hover:text-white hover:bg-white/10 rounded-full transition-colors ml-1">
                      <X size={18}/>
                  </button>
              </motion.div>
          )}
        ...
        <AnimatePresence>
          {modalRenombrarAbierto && (
            <div className="fixed inset-0 z-[100000] flex items-center justify-center bg-slate-950/50 backdrop-blur-md p-4 text-left">
              <motion.div initial={{ scale: 0.95, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.95, opacity: 0, y: 20 }} className="bg-white/95 backdrop-blur-2xl w-full max-w-2xl rounded-[3rem] shadow-2xl flex flex-col overflow-hidden max-h-[85vh] border border-white/80">
                <div className="p-8 border-b border-slate-100 bg-blue-50/80 flex justify-between items-center shrink-0">
                   <div className="flex items-center gap-4">
                      <div className="p-3.5 bg-blue-600 text-white rounded-2xl shadow-sm"><Edit3 size={20} strokeWidth={2.5}/></div>
                      <div>
                        <h2 className="font-black text-xl uppercase tracking-tighter text-blue-950 leading-none">Renombrar Archivos</h2>
                        <p className="text-[10px] text-blue-600 font-bold uppercase tracking-widest mt-1">Editando {datosRenombrar.length} elementos</p>
                      </div>
                   </div>
                   <button onClick={() => setModalRenombrarAbierto(false)} className="p-2.5 text-blue-500 hover:bg-blue-100 rounded-2xl transition-colors"><X size={18}/></button>
                </div>
                
                <div className="flex-1 p-8 overflow-y-auto space-y-4 bg-slate-50/50 custom-scrollbar">
                   {datosRenombrar.map((doc, idx) => (
                      <div key={doc.id} className="bg-white p-5 rounded-3xl border border-slate-200/80 shadow-sm flex items-center gap-5">
                         <div className="w-16 h-16 rounded-2xl bg-slate-100 overflow-hidden flex items-center justify-center shrink-0 border border-slate-200/60">
                         ...
                         <div className="flex-1 space-y-1.5">
                            ...
                            <input type="text" className="w-full bg-slate-50 border border-slate-200/80 p-3.5 rounded-2xl text-xs font-bold text-slate-800 outline-none focus:border-blue-500 focus:ring-4 ring-blue-500/10 transition-all shadow-sm" value={doc.titulo} onChange={(e) => {
                ...
                <div className="p-8 bg-white border-t border-slate-100 shrink-0">
                   <button onClick={guardarNombresMultiples} className="w-full bg-gradient-to-r from-blue-600 to-blue-700 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl shadow-blue-500/25 hover:shadow-blue-500/40 transition-all flex items-center justify-center gap-2 border border-blue-500">
                     <Save size={18} strokeWidth={2.5} /> Guardar Todos los Títulos
                   </button>
                </div>
              </motion.div>
            </div>
            ...
            {visorAbierto && seleccionado && (
              ...
              >
                <div className="lg:w-[80%] w-full h-full relative flex items-center justify-center p-4 lg:p-12 overflow-hidden text-left">
                    ...
                    <button onClick={() => setVisorAbierto(false)} className="absolute top-8 left-8 text-white/60 hover:text-white flex items-center gap-2 font-black text-[10px] uppercase tracking-widest transition-all z-20">
                      <X size={18} strokeWidth={2.5} /> Salir del Visor
                    </button>
                    ...
                    <div className="absolute top-8 right-8 flex gap-3 z-30">
                      <button 
                        ...
                        className="bg-slate-900 text-white p-3 rounded-2xl hover:bg-blue-600 transition-all shadow-lg border border-white/10"
                  ...
                  <div className="flex-1 space-y-8 overflow-y-auto pr-2 text-left custom-scrollbar">
                    ...
                    <div className="space-y-6 pt-10 text-left">
                      <div className="space-y-2 text-left">
                        ...
                        <input type="text" className="w-full bg-slate-50 border border-slate-200/80 p-4 rounded-2xl text-xs font-bold outline-none text-slate-900 focus:ring-4 ring-blue-500/10" value={editando.titulo} onChange={(e) => setEditando({...editando, titulo: e.target.value})} />
                      </div>
                      <div className="space-y-2 text-left">
                        ...
                        <textarea className="w-full bg-slate-50 border border-slate-200/80 p-4 rounded-2xl text-xs font-bold outline-none text-slate-900 focus:ring-4 ring-blue-500/10 min-h-[160px] resize-none" value={editando.descripcion} onChange={(e) => setEditando({...editando, descripcion: e.target.value})}></textarea>
                      </div>
                    </div>
                  </div>
                  <div className="pt-8 space-y-3 text-left">
                    <button onClick={guardarCambios} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl hover:bg-slate-900 transition-all flex items-center justify-center gap-2"><Save size={16} strokeWidth={2.5}/> Guardar Cambios</button>
                    <button onClick={eliminarArchivo} className="w-full bg-red-50 text-red-500 py-4 rounded-2xl font-black text-[10px] uppercase tracking-widest hover:bg-red-500 hover:text-white transition-all flex items-center justify-center gap-2"><Trash2 size={16} strokeWidth={2.5}/> Eliminar Archivo</button>
                  </div>
                </div>
              </motion.div>
            )}
