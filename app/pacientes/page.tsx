            ...
            <table className="w-full border-collapse">
              <thead>
                <tr className="border-b border-slate-50 bg-slate-50/30">
                  <th className="px-4 md:px-10 py-6 text-[10px] font-black text-slate-400 uppercase text-left">Paciente</th>
                  <th className="px-4 md:px-10 py-6 text-[10px] font-black text-slate-400 uppercase text-left hidden sm:table-cell">Identificación</th>
                  <th className="px-4 md:px-10 py-6 text-[10px] font-black text-slate-400 uppercase text-right">Ficha</th>
                </tr>
              </thead>
    ...
    <>
      <tr onClick={onExpand} className={`cursor-pointer transition-all duration-300 ${isExpanded ? 'bg-blue-50/30' : 'hover:bg-slate-50/50'}`}>
        <td className="px-4 md:px-10 py-7">
          <div className="flex items-center gap-3 sm:gap-5 text-left text-slate-900">
            <div className={`w-10 h-10 sm:w-12 sm:h-12 rounded-2xl flex items-center justify-center font-black text-xs transition-all ${isExpanded ? 'bg-blue-600 text-white rotate-6 scale-110 shadow-lg shadow-blue-500/30' : 'bg-slate-100 text-slate-500'} ${!p.activo && !isExpanded && 'bg-red-50 text-red-400'}`}>
              {p.nombre?.[0]}{p.apellido?.[0]}
            </div>
            <div className="text-left text-slate-900">
              <p className={`font-black uppercase text-xs sm:text-sm leading-none mb-1.5 ${!p.activo ? 'text-slate-400 line-through' : 'text-slate-800'}`}>{p.nombre} {p.apellido}</p>
        ...
        <td className="px-4 md:px-10 py-7 text-left text-slate-900 hidden sm:table-cell">
